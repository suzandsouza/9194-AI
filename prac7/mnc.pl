...
% start function and goal function
start([3,3,left,0,0]).
goal([0,0,right,3,3]).

legal(CL,ML,CR,MR) :-
	% is this state a legal one?
	ML>=0, CL>=0, MR>=0, CR>=0,(ML>=CL ; ML=0),(MR>=CR ; MR=0). 
	% i.e the number of cannibals on either side shld not exceed that of the missionaries
...
% Printing
% initally output fun will have a null array
output([]) :- nl. 
